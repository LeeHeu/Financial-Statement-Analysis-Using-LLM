...
class RichDisplay():
    ...
    def report_results(self, research_results: str, excel_results: str):
        ...
        if research_results:
            ...
            self._last_research_results = str(research_results)
        else:
        ...
        if excel_results:
            ...
            self._last_excel_results = str(excel_results)
        else:
            er = "No Excel results!"
    ...
    def write_markdown_report(self):
        """Write a simple markdown report file even when using Rich UX."""
        lines = [
            f"# Deep Research Agent Report - {self.deep_search.ticker}",
            "",
            "## Run Settings",
            f"- Ticker: {self.deep_search.ticker}",
            f"- Company: {self.deep_search.company_name}",
            f"- Provider: {self.deep_search.provider}",
            f"- Orchestrator model: {self.deep_search.orchestrator_model_name}",
            f"- Excel model: {self.deep_search.excel_writer_model_name}",
            f"- Output path: {self.deep_search.output_path}",
            "",
            "## Summary",
            f"- Iterations: {self.orchestrator.iteration}",
            f"- Replans: {self.orchestrator.replan_count}",
            f"- Tasks completed: {len(self.orchestrator.queue.completed_task_names)}",
            f"- Tasks failed: {len(self.orchestrator.queue.failed_task_names)}",
            "",
            "## Research Result (Raw)",
            "```",
            self._last_research_results or "No research results.",
            "```",
            "",
            "## Excel Result (Raw)",
            "```",
            self._last_excel_results or "No Excel results.",
            "```",
            "",
        ]

        with open(self.research_results_file, "w", encoding="utf-8", errors="replace") as file:
            file.write("\n".join(lines))
        self.deep_search.logger.info(
