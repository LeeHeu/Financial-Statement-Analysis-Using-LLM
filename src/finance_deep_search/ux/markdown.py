...
class MarkdownDeepOrchestratorMonitor():
    ...
    def get_status_summary_content(self) -> tuple[MarkdownTable, MarkdownSection]:
...
class MarkdownDisplay():
    ...
    def add_section(
        self, 
        title: str, 
        content: list[MarkdownElement | str] | None = None,
        subsections: dict[str, MarkdownElement] | None = None
    ) -> MarkdownSection:
        content = content or []
    ...
    def __parse_json(self, s: str, context: str = "", log_failure: bool = False) -> tuple[str, list[str] | None]:
        try:
    ...
    def __parse_anthropic_message(self, message_index: int, obj: Any) -> list[Any]:
        ...
        if hasattr(obj, "result") and hasattr(obj, "session_id"):
            rm = obj
    ...
    def __add_results(self, which_one: str, title: str, results: list[Any]) -> MarkdownSection:
        ...
        for i in range(len(results)):
            ...
            subsections[f"reply_{i+1}"] = MarkdownSection(title=f"Reply Message #{i+1}", content=content)
